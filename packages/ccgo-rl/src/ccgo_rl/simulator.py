...
def simulate_benchmark_rows(
    ...
    """Module-level wrapper mirroring the original experiment symbol."""
    simulator = CCGORLSimulator(baseline_offsets=baseline_offsets)
    ...
    return simulator.simulate_benchmark_rows(config)
...
def simulate_counterexamples(
    ...
    """Module-level wrapper exposing counterexample simulation."""
    simulator = CCGORLSimulator(baseline_offsets=baseline_offsets)
    return simulator.simulate_counterexamples(
        seeds=seeds,
        datasets=datasets,
        schedules=schedules,
    )
...
def summarize_by_baseline(
    ...
    """Module-level wrapper returning JSON-friendly summary payloads."""
    simulator = CCGORLSimulator(baseline_offsets=baseline_offsets)
    summary = simulator.summarize_by_baseline(rows=rows, metrics=metrics)
    return simulator.summary_as_dict(summary)
